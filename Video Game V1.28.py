    ...
    while True:
        ...
        if player_alive and not waiting_for_respawn and not enemies_returning:
            ...
            if keys[pygame.K_RIGHT] and player.right < SCREEN_WIDTH:
                player.x += player_speed
            if keys[pygame.K_UP] and player.top > SCREEN_HEIGHT - 200:  # clamp upward
                player.y -= player_speed
            if keys[pygame.K_DOWN] and player.bottom < SCREEN_HEIGHT - 30:  # clamp downward
                player.y += player_speed
...
def show_menu():
    global window  # so we can assign to it here
...
if __name__ == "__main__":
    show_menu()  # Start the game with the main menu