...
PLAYER_WIDTH = 30
PLAYER_HEIGHT = 30
ENEMY_BULLET_COLOR = (173, 216, 230)
...
class Enemy:
    ...
    def shoot(self, bullets):
        pass
    def take_hit(self):
        return True  # basic enemy dies after 1 hit
...
class RedEnemy(Enemy):
    def __init__(self, sx, sy, tx, ty, round_num):
        ...
        self.speed = 2 + 0.1 * round_num
        self.zigzag_amplitude = 20
    ...
    def update_dive(self):
        ...
        offset = int(self.zigzag_amplitude * math.sin(self.zigzag_frequency * self.rect.y + self.zigzag_phase))
        self.rect.x += offset
    ...
    def shoot(self, bullets):
        ...
        if self.in_formation and current_time - self.last_shot_time >= self.shoot_cooldown:
            ...
            bullets.append((bullet, (0, 5), ENEMY_BULLET_COLOR))
...
class GreenEnemy(Enemy):
    ...
    def update_dive(self):
        if self.in_formation:
            self.phase_angle += self.phase_speed
    ...
    def shoot(self, bullets):
        current_time = pygame.time.get_ticks()
        if self.in_formation and current_time - self.last_shot_time >= self.shoot_cooldown:
            left_bullet = pygame.Rect(self.rect.centerx - 6, self.rect.bottom, 4, 12)
            right_bullet = pygame.Rect(self.rect.centerx + 6, self.rect.bottom, 4, 12)
            bullets.append((left_bullet, (-2, 5), ENEMY_BULLET_COLOR))
            bullets.append((right_bullet, (2, 5), ENEMY_BULLET_COLOR))
            self.last_shot_time = current_time
...
class PurpleEnemy(Enemy):
    ...
    def update_dive(self):
        if self.in_formation:
            self.phase_angle += 0.03 * self.speed
            self.rect.x = int(self.center[0] + self.radius * math.cos(self.phase_angle))
            self.rect.y = int(self.center[1] + self.radius * math.sin(self.phase_angle))
    ...
    def shoot(self, bullets):
        current_time = pygame.time.get_ticks()
        if self.in_formation and current_time - self.last_shot_time >= self.shoot_cooldown:
            bullet1 = pygame.Rect(self.rect.centerx - 5, self.rect.bottom, 4, 12)
            bullet2 = pygame.Rect(self.rect.centerx + 5, self.rect.bottom, 4, 12)
            bullets.append((bullet1, (-2, 6), ENEMY_BULLET_COLOR))
            bullets.append((bullet2, (2, 6), ENEMY_BULLET_COLOR))
            self.last_shot_time = current_time
...
def create_wave(round_num):
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            sx = random.randint(-400, SCREEN_WIDTH + 400)
            sy = -100

            if row == 0:
                enemy = PurpleEnemy(sx, sy, x, y, round_num)
            elif row == 1:
                enemy = GreenEnemy(sx, sy, x, y, round_num)
            elif row == 2:
                enemy = RedEnemy(sx, sy, x, y, round_num)
            else:
                enemy = Enemy(sx, sy, x, y, {"color": (255, 255, 0), "points": 10})
...
def create_wave(round_num):
    ...
    for row in range(rows):
        for col in range(cols):
            x = left_offset + col * x_spacing
            y = start_y + row * y_spacing
            start_x = random.randint(-400, SCREEN_WIDTH + 400)
            start_y = -100

            if row == 0:
                enemy = PurpleEnemy(start_x, start_y, x, y, round_num)
            elif row == 1:
                enemy = GreenEnemy(start_x, start_y, x, y, round_num)
            elif row == 2:
                enemy = RedEnemy(start_x, start_y, x, y, round_num)
            else:
                enemy = Enemy(start_x, start_y, x, y, {"color": (255, 255, 0), "points": 10})  # Yellow (basic bee)
...
def show_game_over(score, round_count):
    ...
    over.geometry("400x300")
    over.configure(bg="black")

    label = tk.Label(over, text="GAME OVER", font=("Impact", 32), fg="red", bg="black")
    label.pack(pady=20)

    score_label = tk.Label(over, text=f"Score: {score}\nRound: {round_count}", font=("Arial", 16), fg="white", bg="black")
    ...
    button = tk.Button(over, text="Return to Menu", font=("Arial", 14), command=return_to_menu)
    button.pack(pady=20)
        ...
        if player_alive and not waiting_for_respawn and not enemies_returning:
            ...
            if in_formation_enemies:
                ...
                for e in in_formation_enemies:
                    ...
                    e.target_pos = (e.rect.x, e.rect.y)
...
def main_menu():
    ...
    start_button = tk.Button(window, text="Start", width=20, height=2, command=start_game)
    start_button.pack(pady=5)

    about_button = tk.Button(window, text="About", width=20, height=2, command=show_about)
    about_button.pack(pady=5)

    exit_button = tk.Button(window, text="Exit", width=20, height=2, command=exit_program)
